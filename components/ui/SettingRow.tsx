...
interface SettingRowProps {
  ...
  isMini?: boolean;
  isOverride?: boolean;
  isLocked?: boolean; // NEW: Indicates if the preference is mandatory
  /** The scope currently active for this item */
  activeScope?: PreferenceScope | "DEFAULT";
  onChange: (newValue: any) => void;
  onReset: () => void;
  onToggleLock?: () => void; // NEW: Callback for triggering the lock state
}
...
export const SettingRow: FC<SettingRowProps> = ({
  ...
  // Map backend scope to UI styling - Added Red theme for Locked state
  const getScopeStyles = (scope: string) => {
    if (isLocked && !isMini) return "bg-red-500/10 text-red-600 border-red-500/20";
    ...
    <div
      className={`flex items-center justify-between px-4 transition-all duration-200 group ${
        isMini
          ...
          : `py-4 rounded-xl ${isOverride ? "bg-blue-50/30" : "hover:bg-black/[0.01]"}`
      } ${isLocked && !isMini ? "opacity-90" : ""}`}
    >
      <div className="flex items-center gap-3 min-w-0 overflow-hidden">
        {/* 1. Status Indicator Glow / Lock Dot */}
        {isLocked && !isMini ? (
          ...
          !isMini && <div className="w-1.5 h-1.5 shrink-0 rounded-full bg-black/10" />
        )}
          ...
          <span
            className={`truncate tracking-tight transition-colors ${
              isMini
                ...
                : `text-[13px] font-bold ${isLocked ? "text-red-700/80" : isOverride ? "text-blue-700" : "text-black/80"}`
            }`}
          >
            {label}
            {isLocked && !isMini && <i className="bx bx-lock-alt ml-1 opacity-30 text-[10px]" />}
          </span>
          ...
          {!isMini && (
            <div
              className={`
                ...
                transition-all shrink-0 ${getScopeStyles(activeScope)}
              `}
            >
      ...
      <div className="flex items-center gap-3 shrink-0">
        {/* 3. Input Controls (Disabled if Locked by Authority) */}
        <div className={`flex items-center ${isLocked && !isMini ? "pointer-events-none opacity-50" : ""}`}>
          {type === "switch" && (
            <button
              onClick={() => onChange(!value)}
              className={`relative flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${
          ...
          {type === "select" && (
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <i className={`bx bx-chevron-down transition-transform ${isOpen ? "rotate-180" : ""}`} />
              </button>
              ...
              <AnimatePresence>
                {isOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 z-50 min-w-[140px] bg-white border border-black/5 shadow-xl shadow-black/10 rounded-xl overflow-hidden p-1"
                  >
                    {options?.map((opt) => (
                      <button
                        key={opt.value}
        ...
        {/* 4. Action Area: Reset (for Users) or Lock (for Admins) */}
        <div className="w-8 flex justify-center border-l border-black/[0.05] ml-1">
          {isOverride && !isLocked ? (
            ...
            <Tooltip 
              content={isLocked ? `Mandatory ${activeScope} Policy` : onToggleLock ? (value ? "Lock this policy" : "Enable to lock") : `Value set by ${activeScope}`} 
              side="left"
            >
              <button
                ...
                className={`transition-all ${onToggleLock ? "cursor-pointer hover:scale-110" : "cursor-default opacity-20"}`}
              >
                <i
                  className={`bx ${
                    isLocked ? "bx-lock-alt text-red-500" : onToggleLock ? "bx-lock-open-alt hover:text-black/40" : "bx-lock-open-alt"
                  } text-[14px]`}
                />
              </button>
            </Tooltip>

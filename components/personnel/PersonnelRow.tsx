...
export function PersonnelRow({ personnel, isSelected, onClick }: { personnel: Personnel, isSelected: boolean, onClick: () => void }) {
  ...
  return (
    <div className="flex flex-col w-full border-b border-black/[0.04] last:border-none group overflow-hidden">
      <motion.div 
        ...
        className={`flex items-center px-4 md:px-8 py-3 transition-colors cursor-pointer text-[13px] whitespace-nowrap ${isSelected ? "bg-blue-50/50" : "hover:bg-slate-50/80"}`}
      >
        {/* Staff Code */}
        ...
        <div className="w-[120px] md:w-[160px] shrink-0 flex items-center pr-2 overflow-hidden">
          <div className="flex items-center gap-2 px-2.5 py-1 bg-black/[0.02] border border-black/[0.04] rounded-md max-w-full overflow-hidden">
            ...
            <span className={`text-[11px] truncate ${getBranchColor(true)}`} title={depName}>
              {depName}
            </span>
            {/* Primary Badge for Main Row */}
        ...
        <div className="w-[90px] md:w-[120px] shrink-0 pr-2 overflow-hidden">
          <span className="text-[10px] md:text-[11px] font-semibold text-slate-600 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-wider truncate block w-fit max-w-full" title={personnel.role}>
            {personnel.role}
          </span>
        </div>
        ...
        {hasMultipleAssignments && isExpanded && (
          ...
          >
            {personnel.branchAssignments.map((assignment, idx) => (
              <div key={idx} className="flex items-center px-4 md:px-8 py-2 border-b border-black/[0.02] last:border-none text-[12px] pl-[32px] md:pl-[144px] whitespace-nowrap overflow-hidden">
                <div className="w-[120px] md:w-[160px] shrink-0 flex items-center pr-2 overflow-hidden">
                  <div className="flex items-center gap-2 px-2 py-0.5 opacity-80 max-w-full truncate">
                    ...
                    <span className={`text-[11px] truncate ${getBranchColor(assignment.isPrimary)}`} title={assignment.branch.name}>
                      {assignment.branch.name}
                    </span>
                    {/* Conditional Primary Badge for Assignments List */}
                ...
                <div className="flex-1 min-w-[100px] text-slate-400 italic text-[11px] hidden sm:block truncate pr-2">
                   {assignment.isPrimary ? "Primary Assignment" : "Secondary Assignment"}
                </div>
                <div className="w-[90px] md:w-[120px] shrink-0 overflow-hidden">
                  <span className="text-[10px] font-semibold text-slate-400 bg-black/5 px-1.5 py-0.5 rounded uppercase tracking-wider truncate block w-fit max-w-full" title={assignment.role}>
                    {assignment.role}
                  </span>
                </div>

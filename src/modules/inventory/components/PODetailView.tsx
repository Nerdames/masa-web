...
/* -------------------------
Types - Strictly Aligned with MASA Schema
------------------------- */
interface POItem {
  id: string;
  productId: string;
...
export default function PODetailView({ po, onClose }: PODetailViewProps) {
  ...
  async function submitReceive() {
    ...
    try {
      ...
      dispatch?.({ kind: "PUSH", type: "SUCCESS", title: "Receipt Created", message: `GRN ${body.grnNumber} is awaiting approval.` });
        ...
        <section className="flex flex-col gap-6 md:grid md:grid-cols-2">
          ...
          <div className="space-y-3 md:space-y-4">
             <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2 pt-2">
                <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap">Expected</span>
                ...
                {po.items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
                    <td className="px-3 md:px-4 py-2.5 min-w-[140px]">
                      ...
                      <span className="text-[7.5px] md:text-[9px] bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-slate-500 font-mono">{item.product?.sku}</span>
                    </td>
                    <td className="px-2 md:px-4 py-2.5 text-center text-[9px] md:text-xs text-slate-500 uppercase font-medium">
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between gap-3">
        {isCancelled ? (
          <button disabled className="w-full py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-400 text-[10px] font-bold uppercase rounded-lg border border-slate-200 dark:border-slate-700">
            <AlertOctagon className="w-4 h-4 mr-2 inline" /> Order Voided
          </button>
        ) : isFulfilled ? (
          <button disabled className="w-full py-2.5 bg-slate-100 dark:bg-slate-800 text-emerald-600/60 text-[10px] font-bold uppercase rounded-lg border border-slate-200 dark:border-slate-700">
            <Archive className="w-4 h-4 mr-2 inline" /> Order Fulfilled
          </button>
        ) : hasRejectedReceipt ? (
          ...
          <>
            <div className="flex-1">
              {canVoid ? (
                <button onClick={handleVoid} disabled={isVoiding} className="w-full h-full py-2.5 text-red-600 hover:bg-red-50 text-[10px] font-bold uppercase rounded-lg transition-all border border-transparent hover:border-red-200">
                  {isVoiding ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Void"}
                </button>
              ) : (
                <button disabled className="w-full h-full py-2.5 text-slate-400 bg-slate-50 text-[9px] font-bold uppercase rounded-lg border border-slate-200 cursor-not-allowed flex justify-center items-center gap-1.5">
                  <ShieldAlert className="w-3.5 h-3.5" /> Locked
                </button>
              )}
            </div>
            <button onClick={() => setReceiveOpen(true)} className="flex-[2] py-2.5 bg-slate-900 dark:bg-emerald-600 text-white hover:bg-slate-800 text-[10px] font-bold uppercase rounded-lg shadow-md transition-all active:scale-[0.98]">
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Receive Inventory: {po.poNumber}</h3>
              <button onClick={() => setReceiveOpen(false)} className="p-1.5 hover:bg-slate-100 rounded-md">
            ...
            <div className="p-4 md:p-5 overflow-y-auto space-y-4">
              {receiveItems.map((it, idx) => {
                ...
                return (
                  <div key={it.poItemId} className="p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        <div className="text-[8px] font-bold text-slate-400 uppercase">Remaining</div>
                        <div className="font-bold text-[11px] text-slate-700 dark:text-slate-300">{remaining}</div>
                      </div>
                    </div>
                    <input
            ...
            <div className="p-5 border-t border-slate-100 dark:border-slate-800 flex gap-3">
              <button onClick={() => setReceiveOpen(false)} className="flex-1 py-2 text-xs font-semibold text-slate-500">Cancel</button>
              <button onClick={submitReceive} disabled={isReceiving} className="flex-[2] py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-lg disabled:opacity-50">

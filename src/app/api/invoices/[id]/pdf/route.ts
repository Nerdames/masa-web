...
export async function GET(
  ...
  try {
    ...
    for (const item of invoice.order.items) {
      ...
      doc.text(`${name} — ${qty} × ₦${price.toLocaleString()} = ₦${total.toLocaleString()}`);
    }
    ...
    // Summary
    doc.fontSize(12);
    if (invoice.subtotal != null) doc.text(`Subtotal: ₦${invoice.subtotal.toLocaleString()}`);
    if (invoice.discount && invoice.discount > 0) doc.text(`Discount: ₦${invoice.discount.toLocaleString()}`);
    if (invoice.tax && invoice.tax > 0) doc.text(`Tax: ₦${invoice.tax.toLocaleString()}`);

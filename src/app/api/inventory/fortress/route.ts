...
              message: `Price variables for ${existing.product.name} were directly updated by ${user.role}.`, 
...
            message: `User ${user.id} (${user.role}) requested a price review for ${existing.product.name}.`, 
            activityLogId: log.id,
            approvalId: approvalReq.id, 
            actionTrigger: CriticalAction.PRICE_UPDATE 
          }

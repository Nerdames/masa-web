...
export function useDashboardData() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const statsJson = await safeFetch<StatCardProps[]>("/api/dashboard/stats", []);
      setStats(Array.isArray(statsJson) && statsJson.length ? statsJson : [
        { label: "Orders", value: "0", icon: "bx-cart" },
        { label: "Sales", value: "0", icon: "bx-chart" },
        { label: "Products", value: "0", icon: "bx-box" },
        { label: "Notifications", value: notifications.length.toString(), icon: "bx-bell" },
      ]);
    } finally {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const productsJson = await safeFetch<ChartDataProduct[]>("/api/dashboard/products", []);
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      const notifJson = await safeFetch<{ notifications: Notification[] }>("/api/notifications", { notifications: [] });
  ...
  const fetchDashboardData = useCallback(async () => {
    await Promise.all([fetchStats(), fetchCharts(), fetchProducts(), fetchNotifications()]);

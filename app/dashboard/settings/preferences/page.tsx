...
export default function PreferencesPage() {
  ...
  useEffect(() => {
    if (!role) return;
    setCurrentScope(resolveDefaultScope(role));
  }, [role]);
  ...
  const resolveEffectivePreference = (key: string, target: string | undefined): Preference | undefined => {
    const t = target ?? null;
    return (
  ...
  const handleChange = (setting: HubSetting, value: PreferenceValue): void => {
    setPreferences(prev => [
      ...prev.filter(p => !(p.key === setting.key && p.target === (setting.target ?? null) && p.scope === currentScope)),
      { key: setting.key, value, target: setting.target ?? null, scope: currentScope },
    ]);
  };

  const handleReset = (setting: HubSetting): void => {
    setPreferences(prev => prev.filter(p => !(p.key === setting.key && p.target === (setting.target ?? null) && p.scope === currentScope)));
  };

  const groupedSettings: Record<string, HubSetting[]> = HUB_SETTINGS.reduce((acc, setting) => {
  ...
  return (
    <div className="space-y-6">
      {Object.entries(groupedSettings).map(([section, settings]) => (
        <CollapseSection
          key={section}
          title={section === "dashboard" ? "Summary" : section}
          expanded={!collapsed[section]}
          onToggle={() => setCollapsed(prev => ({ ...prev, [section]: !prev[section] }))}
        >
          <div className="divide-y">
            {settings.map(setting => {
              ...
              return (
                <div key={setting.key} className={`flex items-start justify-between gap-6 px-4 py-4 ${overridden ? "bg-blue-50" : ""}`}>
                  <div className="max-w-md">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <Tooltip content={`Scope: ${pref?.scope}, target: ${pref?.target ?? "global"}`}>
                        <span>{setting.label}</span>
                      </Tooltip>
                      {pref && <ScopeBadge scope={pref.scope} />}
                    </div>
                    {setting.description && <p className="mt-1 text-sm text-gray-500">{setting.description}</p>}
                    {pref && !overridden && <p className="mt-1 text-xs text-gray-400">Inherited from {pref.scope.toLowerCase()}</p>}
                    {overridden && <p className="mt-1 text-xs text-blue-600">Overridden at this scope</p>}
                  </div>

                  <div className="flex items-center gap-3 min-w-[180px]">
                    {pref ? renderControl(setting, pref.value, v => handleChange(setting, v)) : <span className="text-xs italic text-gray-400">Not set</span>}
                    {overridden && (
                      <button onClick={() => handleReset(setting)} className="text-xs text-gray-400 hover:text-gray-700">
                        Reset
                      </button>
                    )}
                  </div>
...
function ScopeBadge({ scope }: { scope: PreferenceScope }) {
  ...
  return <span className={`rounded px-2 py-0.5 text-[10px] font-medium ${styles[scope]}`}>{scope.toLowerCase()}</span>;
}

function renderControl(setting: HubSetting, value: PreferenceValue, onChange: (value: PreferenceValue) => void) {
  if (setting.type === "switch") return <SmartSwitch checked={Boolean(value)} onChange={onChange} />;
  if (setting.type === "select") return (
    <select value={String(value)} onChange={e => onChange(e.target.value)} className="rounded-md border px-2 py-1 text-sm">
...
function SmartSwitch(props: { checked: boolean; onChange: (checked: boolean) => void }) {
  const { checked, onChange } = props;
  return (
    <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 ${checked ? "bg-blue-600" : "bg-gray-300"}`}>
      <span aria-hidden="true" className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200 ease-in-out ${checked ? "translate-x-6" : "translate-x-1"}`} />
    </button>
  );
}

function CenteredMessage({ children }: { children: React.ReactNode }) {
  return <div className="flex min-h-[60vh] items-center justify-center text-sm text-gray-500">{children}</div>;
}